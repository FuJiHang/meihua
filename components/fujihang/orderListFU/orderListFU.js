          ...
          success(res) {
            if (res.confirm) {
              app.fg({
                ...
                app.fh()
                if (r.data.Status == "OK") {
                  r.data.Data.Status==2?wx.navigateTo({
                    url: "/pages/fujihang/fuPayState/fuPayState?money=" + that.data.order.Amount + "&orderData=" + encodeURIComponent(JSON.stringify(that.data.order)) + "&des=" + that.data.order.LineItems[0].Description
                  }):app.fa('未支付成功，请重试')
