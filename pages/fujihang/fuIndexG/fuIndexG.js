...
Page({
  ...
  toPYQFNT(e) {
    ...
    setTimeout(() => {
      ...
      let chuang = ''
      if (WeiZhi % 10 == 0) {
        // console.log(data.getXKTListYS.slice(WeiZhi, ((Math.ceil(WeiZhi / 10) + 1) * 10)))
        chuang = encodeURIComponent(JSON.stringify(data.getXKTListYS.slice(WeiZhi, ((Math.ceil(WeiZhi / 10) + 1) * 10))));
      }
      else{
        // console.log(data.getXKTListYS.slice(WeiZhi, (Math.ceil(WeiZhi / 10) * 10)),'=====')
