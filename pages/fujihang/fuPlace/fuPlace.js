...
Page({
  ...
  data: {
    ...
    isShuaLian: false,//能否刷脸
    chooseShuaLian: false,//是否开启刷脸
  },
  // 关闭协议
  closeXY() {
    this.setData({
  ...
  coupon() {
    let that = this
    wx.navigateTo({
      url: '/pages/fujihang/fuCoupon/fuCoupon?price=' + this.data.jisuanPrice + "&StoreId=" + this.data.storeData.StoreId
    })
  },
  ...
  chooseShuaLianFN() {
    this.setData({
      chooseShuaLian: !this.data.chooseShuaLian
    })
  },
                      ...
                      success(resw) {
                        if (resw.confirm) {
                          app.fg({
                            ...
                            app.fh()
                            if (p.data.Status == "OK") {
                              p.data.Data.Status == 2 ? that.okzhongji(data, r) :wx.showModal({
                                title: '提示',
                                content: '未完成支付，是否跳转订单列表',
                                success() {
                                  wx.switchTab({
