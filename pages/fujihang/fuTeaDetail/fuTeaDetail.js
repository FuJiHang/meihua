    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          // });
          app.globalData.fuStoreId=data.orderData.StoreId
          wx.reLaunch({
  ...
  coupon(){
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          wx.navigateTo({
            url:'/pages/fujihang/fuCoupon/fuCoupon?price='+that.data.orderData.OrderTotal+"&type=1&StoreId="+that.data.orderData.StoreId
          })
        }
      }
    })
